...
class CoordinatePlaneRenderer {
    ...
    redrawPlane() {
        ...
        this.ctx.moveTo(512 + (this.xres - 0.5) * this.size, 512 - this.size * 0.5);
        this.ctx.lineTo(512 + this.xres * this.size, 512);
        this.ctx.lineTo(512 + (this.xres - 0.5) * this.size, 512 + this.size * 0.5);
        this.ctx.stroke();
        // Y Axis
        ...
        this.ctx.moveTo(512 - this.size * 0.5, 512 - (this.yres - 0.5) * this.size);
        this.ctx.lineTo(512, 512 - this.yres * this.size);
        this.ctx.lineTo(512 + this.size * 0.5, 512 - (this.yres - 0.5) * this.size);
        this.ctx.stroke();
    }
    renderCells() {
     ...
     * @param {Point} point
     */
    drawPoint(point, interpolate = true) {
        if (interpolate) {
            point.interpolateX();
            point.interpolateY();
        }
...
class LinesRenderer {
    ...
    renderLines(pointArr) {
        ...
        for (var i = 0; i < pointArr.length; i++) {
            ...
            this.ctx.beginPath();
            this.ctx.moveTo(512 + pointArr[i].x * this.cplane.size, 512 - pointArr[i].y * this.cplane.size);
            this.ctx.lineTo(512 + pointArr[il].x * this.cplane.size, 512 - pointArr[il].y * this.cplane.size);
            this.ctx.stroke();
        }
        this.ctx.restore();
    }
...
class CursorPointRenderer {
    ...
    drawNearPoint() {
        ...
        if ((this.mx && this.my) != undefined) {
            ...
            this.ctx.beginPath();
            this.ctx.arc(512 + this.x * this.cplane.size, 512 - this.y * this.cplane.size, 5, 0, Math.PI * 2);
            this.ctx.fill();
        }
        this.ctx.restore();
    }
